            ...
            <flux:table container:class="max-h-[70vh] min-w-[640px]">
                ...
                <flux:table.rows>
                    @forelse($posts as $post)
                        <flux:table.row>
                            <flux:table.cell>
                                <flux:link :href="route('admin.posts.show', $post)" wire:navigate class="font-medium">
                                    {{ $post->title }}
                                </flux:link>
                            </flux:table.cell>
                            ...
                            <flux:table.cell class="text-end">
                                <flux:button size="sm" variant="ghost" :href="route('admin.posts.show', $post)" wire:navigate>
                                    {{ __('View') }}
                                </flux:button>
                                @if($post->isPublished())
